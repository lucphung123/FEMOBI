<template>
  <div
    class="fixed flex justify-center bg-[#000000da] inset-0 z-[1000] overflow-y-auto"
    @click="$emit('close-modal')"
  >
    <div
      @click.stop
      class="text-center bg-[white] w-250 mt-10 absolute px-0 py-10 rounded-[20px]"
    >
      <h6 class="font-medium text-[36px] mx-0 font-bold">
      ...
      <div class="grid grid-cols-4 gap-5 p-5">
        ...
        >
          <p
            class="absolute -translate-x-2/4 -translate-y-2/4 z-1 w-full text-[white] font-bold text-lg leading-6 text-center left-2/4 top-2/4"
          >
            {{ data.label }}
          </p>
          <div
          ...
          <div
            class="w-5 h-5 top-1 right-1 rounded-full absolute border flex justify-center items-center"
            :class="{ 'bg-[#f1b821]': isChecked[index] }"
          >
      ...
      <button
        class="hover-bg-[#f1b821] hover-text-[#1d1d1b] justify-center mt-5 bg-[#004390] h-10 text-[white] text-sm px-[20px] rounded-[20px] font-bold"
      >
        CHỌN HOÀN TÂT
      </button>
    </div>
