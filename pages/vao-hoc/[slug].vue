      ...
      <div class="absolute w-screen left-1/2 -translate-x-1/2 -translate-y-full bg-white bg-cover bg-top h-300 flex flex-col items-center">
        <div class="w-full lg:w-225 xl:(w-320 h-250px) 2xl:(w-340 h-280px) my-auto">
          ...
          <div class="flex border rounded-xl mt-5">
            <div class="w-60% h-120">
              <div v-if="selectedVideo" class="mb-4">
                <video width="770" height="400" controls autoplay v-if="!isYoutubeVideo(selectedVideo)">
                  <source :src="selectedVideo" type="video/mp4" />
                </video>
                <iframe width="770" height="400" :src="convertToEmbedUrl(selectedVideo)" frameborder="0" allowfullscreen v-if="isYoutubeVideo(selectedVideo)"></iframe>
              </div>
            </div>
            <div class="w-40% p-10">
              <div class="flex items-center gap-3 text-5 text-color_4">
                <div class="i-mdi:television-play"></div>
                <p>Nội dung học</p>
              </div>
              <div v-for="(lesson, index) in detailData.data.lesson" :key="lesson.id">
                <div class="flex justify-between items-center mt-5 border-b pb-5">
                  <div class="text-black cursor-pointer w-full font-bold text-6 text-left outline-none text-base" @click="toggle(index)">
                    {{ lesson.label }}
                  </div>
                  <div :class="arrowIcon(index)"></div>
                </div>
                <div :class="contentClasses(index)">
                  <div class="bg-white" v-for="child in lesson.children" :key="child.id">
                    <span class="cursor-pointer bg-white leading-15 mt-10" @click="showVideo(child)">{{ child.label }}</span>
                  </div>
                </div>
              </div>
            </div>
...
const contentClasses = sectionIndex => {
  const baseClasses = "overflow-hidden transition-max-height ease-out duration-800 "
...
const { data: detailData } = await RestApi.course.getDetailsFull({
  query: query,
})
